...
def make_alembic_config(
        cmd_opts: Namespace, base_path: str | Path = settings.project_root
) -> AlembicConfig:
    # Заменяем путь для alembic.ini на абсолютный
    ...
    # Заменяем путь к папке alembic на абсолютный
...
def alembic_config_from_url(pg_url: str = None) -> AlembicConfig:
    '''Объект представления файла alembic.ini.'''
    cmd_options = Namespace(
        config='alembic.ini', # Файл конфигурации
        name='alembic',
        pg_url=pg_url,
        raiserr=True,
        x=None,
...
async def tmp_database(db_url: URL, suffix: str = '', **kwargs) -> AsyncIterator[str]:
    '''Контекстменеджер для создания новой базы данных.'''
    tmp_db_name = '.'.join([uuid.uuid4().hex, 'test-base', suffix])
